...
$query = "
            INSERT INTO `hero` (`title`, `desc` `buttonName`) 
...
$query = "UPDATE `hero` SET `title`='$title',`desc`='$desc', `buttonName`='$buttonName'  WHERE `id` = '$id' ";
...
if($crud == 'delete'){
  ...
  $sql = $conn->prepare("DELETE FROM `hero` WHERE id=?");
  $sql->bind_param("s", $id);
  $sql->execute();
