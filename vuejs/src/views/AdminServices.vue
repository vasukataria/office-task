...
<div class="myModal" v-if="showDeleteModal">
	<div class="modalContainer">
		...
		<div class="modalBody">
			...
			<h2 class="text-center">{{clickVideo.class}} {{clickVideo.title}} {{clickVideo.desc}}</h2>
		</div>
		<hr>
		<div class="modalFooter">
			<div class="footerBtn pull-right">
...
export default{
	...
	methods:{
		getAllMembers: function(){
			var self = this;
			axios.get('http://localhost/officetask/database/service.php')
		...
		saveMember: function(){
			...
			axios.post('http://localhost/officetask/database/about.php?crud=create', this.newMember)
				.then(function(response){
					console.log(response);
					self.newMember = {class: '', title: '', desc:''};
					if(response.data.error){
						self.errorMessage = response.data.message;
					}
					else{
