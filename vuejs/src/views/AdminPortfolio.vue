...
<div class="myModal" v-if="showDeleteModal">
	<div class="modalContainer">
		...
		<div class="modalBody">
			...
			<h2 class="text-center"><img :src="'data:image.png;base64,' + clickVideo.img" alt=""><br> {{clickVideo.title}}<br> {{clickVideo.desc}}</h2>
		</div>
		<hr>
		<div class="modalFooter">
			<div class="footerBtn pull-right">
...
export default{
	...
	methods:{
		getAllMembers: function(){
			var self = this;
			axios.get('http://localhost/officetask/database/portfolio.php')
		...
		saveMember: function(){
			...
			var self = this;
            //debugger;		
			axios.post('http://localhost/officetask/database/portfolio.php?crud=create', this.newMember)
				.then(function(response){
					console.log(response);
					self.newMember = {img: '', title: '', desc:''};
					if(response.data.error){
						self.errorMessage = response.data.message;
					}
					else{
		...
		updateMember(){
			...
			axios.post('http://localhost/officetask/database/portfolio.php?crud=update', memForm)
				.then(function(response){
					console.log(response);
					self.clickVideo = {};
					if(response.data.error){
		...
		deleteMember(){
			var self = this;
			var memForm = self.toFormData(self.clickVideo);
			axios.post('http://localhost/officetask/database/portfolio.php?crud=delete', memForm)
				.then(function(response){
					//console.log(response);
					self.clickVideo = {};
					if(response.data.error){
