			...
			<table class="table table-bordered table-striped">
				...
				<tbody>
					<tr v-for="video in videos"  v-bind:key="video.id">
						<td>{{ video.class }}</td>
						<td>{{ video.ChildTitle }}</td>
						<td>{{ video.ChildDesc }}</td>
						<td>
	...
	<div class="modalContainer">
		...
		<div class="modalBody">
			<div class="form-group">
				...
				<input type="text" class="form-control" v-model="newMember.class" name="class">
			</div>
			<div class="form-group">
				<label>Title:</label>
				<input type="ChildTitle" class="form-control" v-model="newMember.ChildTitle" name = "ChildTitle">
			</div>
			<div class="form-group">
				<label>Desc:</label>
				<input type="text" class="form-control" v-model="newMember.ChildDesc" name = "ChildDesc">
			</div>
		</div>
		<hr>
...
<div class="myModal" v-if="showEditModal">
	<div class="modalContainer">
		...
		<div class="modalBody">
			<div class="form-group">
				<label>Icon class:</label>
				<input type="text" class="form-control" v-model="clickVideo.class">
			</div>
			<div class="form-group">
				<label>Title:</label>
				<input type="text" class="form-control" v-model="clickVideo.ChildTitle">
			</div>
            <div class="form-group">
                <label>Desc:</label>
                <input type="text" class="form-control" v-model="clickVideo.ChildDesc">
            </div>
		</div>
		<hr>
		<div class="modalFooter">
...
<div class="myModal" v-if="showDeleteModal">
	<div class="modalContainer">
		...
		<div class="modalBody">
			...
			<h2 class="text-center">{{clickVideo.class}} {{clickVideo.ChildTitle}} {{clickVideo.ChildDesc}}</h2>
		</div>
		<hr>
		<div class="modalFooter">
			<div class="footerBtn pull-right">
...
export default{
	...
	data(){
		...
		successMessage: "",
		videos: [],
		newMember: {class: '', ChildTitle: '', ChildDesc:''},
		clickVideo: {}
	}
},
	...
	methods:{
		getAllMembers: function(){
			var self = this;
			axios.get('http://localhost/officetask/database/about.php')
		...
		saveMember: function(){
			...
			var self = this;
            //debugger;		
			axios.post('http://localhost/officetask/database/about.php?crud=create', this.newMember)
				.then(function(response){
					console.log(response);
					self.newMember = {class: '', ChildTitle: '', ChildDesc:''};
					if(response.data.error){
						self.errorMessage = response.data.message;
					}
					else{
		...
		updateMember(){
			...
			axios.post('http://localhost/officetask/database/about.php?crud=update', memForm)
				.then(function(response){
					console.log(response);
					self.clickVideo = {};
					if(response.data.error){
