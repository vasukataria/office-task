			...
			<table class="table table-bordered table-striped">
				...
				<tbody>
					<tr v-for="video in videos"  v-bind:key="video.id">
						...
						<td>{{ video.desc }}</td>
						<td>{{ video.Clients }}</td>
						<td>{{ video.Projects }}</td>
						<td>{{ video.HoursOfSupport }}</td>
						<td>{{ video.HardWorkers }}</td>
						<td>
	...
	<div class="modalContainer">
		...
		<div class="modalBody">
			<div class="form-group">
				<label>title:</label>
				<input type="text" class="form-control" v-model="newMember.title" name="title">
			</div>
			<div class="form-group">
				<label>desc:</label>
				<input type="desc" class="form-control" v-model="newMember.desc" name = "desc">
			</div>
			<div class="form-group">
				<label>Projects:</label>
				<input type="text" class="form-control" v-model="newMember.Projects" name = "Projects">
			</div>
			<div class="form-group">
				<label>Clients:</label>
				<input type="text" class="form-control" v-model="newMember.Clients" name ="Clients">
			</div>
			<div class="form-group">
				<label>Hours Of Support:</label>
				<input type="text" class="form-control" v-model="newMember.HoursOfSupport" name ="HoursOfSupport">
			</div>
			<div class="form-group">
				<label>Hard Workers:</label>
				<input type="text" class="form-control" v-model="newMember.HardWorkers" name ="HardWorkers">
			</div>
		</div>
		<hr>
...
<div class="myModal" v-if="showEditModal">
	<div class="modalContainer">
		...
		<div class="modalBody">
            ...
            <div class="form-group">
                <label>Projects:</label>
                <input type="text" class="form-control" v-model="clickVideo.Projects">
            </div>
            <div class="form-group">
                <label>Clients:</label>
                <input type="text" class="form-control" v-model="clickVideo.Clients">
            </div>
            <div class="form-group">
                <label>Hours Of Support:</label>
                <input type="text" class="form-control" v-model="clickVideo.HoursOfSupport">
            </div><div class="form-group">
                <label>Hard Workers:</label>
                <input type="text" class="form-control" v-model="clickVideo.HardWorkers">
            </div>
		</div>
		<hr>
		<div class="modalFooter">
...
<div class="myModal" v-if="showDeleteModal">
	<div class="modalContainer">
		...
		<div class="modalBody">
			...
			<h2 class="text-center">{{clickVideo.title}} {{clickVideo.name}} {{clickVideo.Projects}} {{clickVideo.Clients}} {{clickVideo.HoursOfSupport}}{{clickVideo.HardWorkers}}</h2>
		</div>
		<hr>
		<div class="modalFooter">
			<div class="footerBtn pull-right">
...
export default{
	...
	data(){
		...
		successMessage: "",
		videos: [],
		newMember: {title: '', desc: '', Projects:'', Clients:'', HoursOfSupport:'', HardWorkers:''},
		clickVideo: {}
	}
},
	...
	methods:{
		getAllMembers: function(){
			var self = this;
			axios.get('http://localhost/officetask/database/fact.php')
		...
		saveMember: function(){
			...
			var self = this;
            //debugger;		
			axios.post('http://localhost/officetask/database/fact.php?crud=create', this.newMember)
				.then(function(response){
					console.log(response);
					self.newMember = {title: '', desc: '', Clients:'', Projects:'', HoursOfSupport:'', HardWorkers:''};
					if(response.data.error){
						self.errorMessage = response.data.message;
					}
					else{
		...
		updateMember(){
			...
			axios.post('http://localhost/officetask/database/fact.php?crud=update', memForm)
				.then(function(response){
					console.log(response);
					self.clickVideo = {};
					if(response.data.error){
		...
		deleteMember(){
			var self = this;
			var memForm = self.toFormData(self.clickVideo);
			axios.post('http://localhost/officetask/database/fact.php?crud=delete', memForm)
				.then(function(response){
					//console.log(response);
					self.clickVideo = {};
					if(response.data.error){
